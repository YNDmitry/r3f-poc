...
import { PerspectiveCamera, OrbitControls, useGLTF, Stats, Environment, Preload } from '@react-three/drei'
...
import { SceneConfigProvider, useSceneConfig } from './config/SceneConfigContext'
import type { SceneConfiguration } from './config/SceneConfigContext'
...
function Intro({ controlsRef, cameraZ }: { controlsRef: React.RefObject<OrbitControlsImpl | null>, cameraZ: number }) {
...
function SceneContent({ modelA, modelB }: { modelA: string, modelB: string }) {
  ...
  useEffect(() => {
    if (mode === 'focus-a' || mode === 'focus-b') {
      const container = document.querySelector('[data-tres="scene"]') || document.querySelector('[data-tres="hero-duo"]');
      if (container) {
  ...
  return (
    <>
      ...
      {import.meta.env.DEV && <Screenshotter />}
      ...
      <group position={stagePos}>
        <Suspense fallback={null}>
          <Product
            ...
            onPointerOver={() => { if (mode === 'grid') document.body.style.cursor = 'pointer' }}
