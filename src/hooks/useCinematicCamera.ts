...
export function useCinematicCamera(
  ...
  useFrame((state) => {
    ...
    if (isAnimating || isHolding) {
      ...
      controls.target.set(
        THREE.MathUtils.lerp(stateRef.current.startTarget.x, stateRef.current.endTarget.x + swayTargetX, t),
        THREE.MathUtils.lerp(stateRef.current.startTarget.y, stateRef.current.endTarget.y + swayTargetY, t),
